...
A Subscriber indicates that the User's session has ended, perhaps by exiting the Subscriber through a logout, session time-out or other reason. Upon receiving a `UserLogout` event, the Subscriber SHOULD logout the user or respond with a `SyncError`.
...
| Version | Description
| ---- | ----
| 1.0 | [Initial Release](https://fhircast.hl7.org/events/userlogout/)
