...
The Hub distributes the corresponding event to all Subscribers. The Hub replaces the `context.versionId` in the request with a new `context.versionId` generated by the Hub.  The prior version, `context.priorVersionId` of the context enables continuity.  If the value of `context.priorVersionId` is not in agreement with the `context.versionId` last received by a Subscriber, the Subscriber can [retreive the current context](2-9-GetCurrentContext.html). 
