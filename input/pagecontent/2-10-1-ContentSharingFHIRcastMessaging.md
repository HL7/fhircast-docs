...
Commonly all information relevant to the update is contained in an entry’s resource (i.e., information is passed by value). For example, an `Observation` resource usually contains all information regarding that observation. However, it is also possible to pass content by reference as is specified in the base FHIR standard (see [FHIRcast Content Update Bundle](http://hl7.org/fhir/http.html#transaction) and corresponding examples). Further implementer experimentation is needed for increased use of references to resources not contained in the `Bundle`.  
