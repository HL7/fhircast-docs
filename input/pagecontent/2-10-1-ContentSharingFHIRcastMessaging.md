...
When a Subscriber creates a FHIR resource which it asks be added to the anchor context's content, it must create an `id` for the resource (see: [Resource.id](http://hl7.org/fhir/resource.html)).  The Subscriber SHOULD use a mechanism to generate the `id` such that it will be globally unique (e.g., a [UUID](https://en.wikipedia.org/wiki/Universally_unique_identifier)).  If at some later point in time the resource is added to an information sharing session by any Subscriber (for the same or a different anchor context) it SHOULD be added with the same `id` with which it was originally created.
