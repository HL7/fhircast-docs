...
`hub.events` | Conditional | All | *string* | Required for `subscribe` requests, SHALL NOT be present for `unsubscribe` requests. Comma-separated list of event types from the Event Catalog for which the Subscriber wants to subscribe. Hubs SHALL ignore repeated elements. Partial unsubscribe requests are not supported and SHALL result in a full unsubscribe.
