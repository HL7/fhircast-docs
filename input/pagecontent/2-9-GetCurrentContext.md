...
At any point in time the Hub may contain multiple contexts in a given topic.  Each context is established with a *-open request and remains in the Hub's record of contexts until a *-close request is accepted for the context.  If one of the established contexts is closed without another being opened, the Hub SHALL return an empty context (which as specified above is indicated by returning an empty string for the `context.type` and an empty array in `context`).  See [Multi-tab Considerations](4-4-multitab-considerations.html) for additional discussion on multiple context scenarios.
